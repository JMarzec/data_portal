...
################################################################################
#
#   Description: Script performing clustering and generating heatmap for user-defined genes using relative linear copy-number values. The pipeline calls gained and lost genes by estimating the putative copy-number alterations from the linear copy-number values. It assigns gain for linear CN values above 0.5 and loss for linear CN values below -0.5. The script imputes missing data for genes with non-missing data in >50% of samples. Genes with >50% missing data are removed prior clustering. The heatmap presents only the 500 genes with the highest copy-number variance across samples. Missing values should be denoted as 'NA'. NOTE: the script allowes to process gene matrix with duplicated gene IDs.
#
#   Command line use example: R --file=./CN_binary_heatmap.R --args "CCLE_PC_CN_processed.txt" "CCLE_PC_target.txt" "Example_results/PC_CN_binary_heatmap" "Gene_list.txt" "3" "10"
#
...
##### Use amelia function to impute missing data. NOTE: the parameter 'empri' is set to the number of rows of the imput data. If error is produces by 'amelia' funciton try to increase this number
...
pdf(paste(coreName, "hist.pdf", sep = "_"), width = 8, height = 6, pointsize = 12)
histInfo <- hist(data.matrix(expData), breaks=35, plot=FALSE)

hist(data.matrix(expData), col="grey", xlab="Relative linear copy-number values", main="", breaks=35, xlim=c(-3,3), labels = TRUE, ylim=c(0,(max(histInfo$counts)+1)))
dev.off()


##### Draw histogram of correlation coefficients (PLOTLY)
p <- plot_ly(x = ~unlist(expData), type = 'histogram', nbinsx = 200, width = 800, height = 500) %>%
...
htmlwidgets::saveWidget(as_widget(p), paste(coreName, "hist.html", sep = "_"))


##### Assign gain for linear CN values above 0.5 and loss for linear CN values below -0.5
expData[ expData > 0.5 ] <- 1
expData[ expData < -0.5 ] <- -1
expData[ expData <= 0.5 & expData >= -0.5 ] <- 0
...
htmlwidgets::saveWidget(as_widget(p), paste(coreName, "binary_heatmap.html", sep = "_"))
