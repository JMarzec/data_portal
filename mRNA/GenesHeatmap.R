...
#
#   Command line use example: R --file=./GenesHeatmap.R --args "CCLE_PC_processed_mRNA.txt" "CCLE_PC_target.txt" "Example_results/PC_GenesHeatmap" "Gene_list.txt" "3" "3"
#
...
##### Keep 500 genes with the highest expression variance across samples
...
if ( nrow(expData) < 200 ) {
    ...
    sel<-order(rsd, decreasing=TRUE)[1:200]
}
...
Annotation <- as.matrix(annData[,c(2:ncol(annData))])
names(Annotation) <- names(annData)[c(2:ncol(annData))]
