...
#
#   Command line use example: R --file=./Expr_CN_mut_Profile.R --args "CCLE_PC_processed_mRNA.txt" "CCLE_PC_processed_CN.txt" "CCLE_PC_processed_mut.txt" "CCLE_target.txt" "KRAS" "Example_results/PC_Expr_CN_mut_Profile"
#
...
##### Initiate varianle for the gene mutation status for each sample
...
pdf(paste(coreName, gene, "mRNA_vs_CN_mut_plot.pdf", sep = "_"), pointsize = 12)
...
htmlwidgets::saveWidget(as_widget(p), paste(coreName, gene, "mRNA_vs_CN_mut_plot.html", sep = "_"))
...
##### Generate box-plot
pdf(paste(coreName, gene, "mRNA_vs_CN_mut_boxplot.pdf", sep = "_"), pointsize = 12 ,width = 2*length(unique(as.numeric(gene.cn))), height = 5)
#par(mar=c(3, 3, 3, 2))
...
##### Save the box-plot as html (PLOTLY)
htmlwidgets::saveWidget(as_widget(p), paste(coreName, gene, "mRNA_vs_CN_mut_boxplot.html", sep = "_"))
